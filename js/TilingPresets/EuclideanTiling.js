Tiling.EuclideanTiling = function ({size}={}) {
	var tils = [];
		...
		for(var k=-size;k<=size;k++) {
			for(var i=-size;i<=size;i++) {
				if(z+k+i==0) {
					...
					tils.push(upTriangle(3*i+2*k,2-i-3*k,3,triNei1));
					tils.push(upTriangle(-1+3*i+2*k,-1-i-3*k,0,triNei2));
					tils.push(upTriangle(1+3*i+2*k,-1-i-3*k,0,triNei3));
			
					tils.push(downTriangle(3*i+2*k,-i-3*k,0,triNei4));
					tils.push(downTriangle(-2+3*i+2*k,-i-3*k,0,triNei5));
					tils.push(downTriangle(3*i+2*k,-2-i-3*k,0,triNei6));
		
					tils.push(hexTile((5/3)*i+k/3,-i-3*k,0,hexNeighbors));
		
					tils.push(upTriangle(1+3*i+2*k,-i-3*k,0,triUp));
					tils.push(downTriangle(-1+3*i+2*k,1-i-3*k,0,triDo));
				}
			}
		}
	}
...
function hexTile(x, y, z, neighbors){
	var id = [x, y, z];
...
function upTriangle(x, y, n, neighbors){
	var id = [x, y, "up"];
	let sq3 = Math.sqrt(3);
	var bounds = [];
...
function downTriangle(x, y, n, neighbors){
	var id = [x, y, "down"];
	let sq3 = Math.sqrt(3);
	var bounds = [];
	bounds.push(x+ (y-n)/2 + 0.5, (y+1)*(sq3/2) - n*(sq3/6));
